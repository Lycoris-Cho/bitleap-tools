...
export default function ImageCompressPage() {
    ...
    canvas.toBlob(
      (blob) => {
        ...
        if (blob.size > originalSize) {
          setErrorMsg('⚠️ 压缩后文件比原图更大！建议调大压缩强度或改用 WebP。PNG 为无损格式，体积通常更大。')
        } else {
          setErrorMsg('')
        }
      },
  ...
  return (
    <div className="max-w-3xl mx-auto py-12 px-4 sm:px-6 lg:px-10">
      <Breadcrumb />
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight mb-2">图片压缩</h1>
        <p className="text-app-muted text-sm">在浏览器中压缩 JPG / PNG / WebP，完全本地处理，不上传服务器</p>
      </div>
          ...
          {file && preview ? (
            <div>
              ...
              <div className="mt-3 text-sm">
                ...
                <div className="text-xs text-app-muted mt-2">点击此处可重新选择图片</div>
              </div>
            </div>
          ) : (
            <>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
          <div>
            <div className="flex justify-between text-sm mb-2">
              <span className="text-gray-700 font-medium">压缩强度</span>
              <span className="font-mono text-violet-600 font-semibold">{compressStrength}</span>
            </div>
            <input
              type="range"
              min={1}
      ...
      {file && (
        <div className="flex gap-4 mb-10">
          {!compressed && (
            <button
              onClick={compress}
              className="px-6 py-2.5 bg-violet-500 text-white rounded-xl hover:bg-violet-600 active:scale-95 transition-all font-medium shadow-sm shadow-violet-500/20"
            >
              压缩图片
            </button>
          )}
          <button
            onClick={handleReset}
            className="px-6 py-2.5 bg-orange-50 text-orange-600 border border-orange-200 rounded-xl hover:bg-orange-100 active:scale-95 transition-all font-medium"
          >
            重置
          </button>
        </div>
      ...
      {preview && compressed && (
        <div className="mb-10 space-y-8">
            ...
            <div>
              <div className="text-sm font-medium text-gray-700 mb-2">
                ...
                {originalSize > compressedSize ? (
                  ...
                  <span className="ml-2 text-red-500 font-medium">体积增大</span>
                )}
              </div>
              <img
                src={compressed}
          ...
          <button
            onClick={download}
            className="px-6 py-2.5 bg-blue-500 text-white rounded-xl hover:bg-blue-600 active:scale-95 transition-all font-medium"
          >
            下载压缩图片
          </button>
        </div>
      ...
      <div className="mt-10 p-4 bg-gray-50 border border-app-border rounded-xl">
        ...
        <ul className="text-xs text-app-muted space-y-1.5 leading-relaxed">
          <li>• 所有压缩操作均在浏览器本地完成，图片不会上传到任何服务器</li>
          <li>• 压缩强度越大，文件体积越小，但画质损失越大</li>
          <li>• WebP 相同条件下压缩效果最优；PNG 为无损输出，可能会比原图更大</li>
          <li>• 支持 JPG / PNG / WebP 格式，单文件最大 10MB</li>
        </ul>
      </div>
