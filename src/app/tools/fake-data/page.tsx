...
export default function FakeData() {
  ...
  return (
    <div className="flex w-full h-[calc(100vh-4rem)] overflow-hidden bg-gray-50/50">
      {/* 左侧配置 */}
      <div className="w-48 sm:w-56 lg:w-64 shrink-0 h-full overflow-y-auto border-r border-app-border/60 bg-app-bg/80 backdrop-blur-xl p-4 space-y-5">
        <div>
          ...
          <div className="flex gap-2">
            <input
              ...
              className="min-w-0 flex-1 px-3 py-2 text-sm border border-app-border rounded-xl font-mono focus:outline-none focus:ring-2 focus:ring-gray-900"
            />
            <button
              onClick={() => setSeed(Math.random().toString(36).slice(2, 8))}
              className="shrink-0 px-2.5 py-2 text-xs bg-gray-100 text-app-muted rounded-xl hover:bg-gray-200 active:scale-95 transition"
            >
              随机
            </button>
          </div>
          ...
          {results.length === 0 ? (
            ...
            <>
              ...
              <div className="flex gap-2 flex-wrap">
                <button onClick={exportJson} className="px-4 py-2 text-xs bg-app-bg border border-app-border rounded-lg hover:bg-gray-50 active:scale-95 transition">
                  {copied === 'json' ? '✓ 已复制' : '📋 复制 JSON'}
                </button>
                <button onClick={exportCsv} className="px-4 py-2 text-xs bg-app-bg border border-app-border rounded-lg hover:bg-gray-50 active:scale-95 transition">
                  {copied === 'csv' ? '✓ 已复制' : '📋 复制 CSV'}
                </button>
                <button onClick={exportSql} className="px-4 py-2 text-xs bg-app-bg border border-app-border rounded-lg hover:bg-gray-50 active:scale-95 transition">
