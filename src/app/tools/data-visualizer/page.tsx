...
const SAMPLES: Record<string, string> = {
  ...
  scatter: (() => { const rows = ['x,y,group']; for (let i = 0; i < 30; i++) { const g = i % 3; rows.push(`${(Math.random() * 100).toFixed(1)},${(Math.random() * 100).toFixed(1)},组别${g + 1}`) } return rows.join('\n') })(),
...
function detectFields(data: Record<string, unknown>[]): FieldMeta[] {
  ...
  return Object.keys(data[0]).map((k) => {
    ...
    let t: FieldMeta['inferredType'] = 'string'
    if (!isNaN(Number(sample)) && sample.trim() !== '') t = 'number'
    else if (!isNaN(Date.parse(sample))) t = 'date'
    return { name: k, sample, inferredType: t }
  })
}
...
function inferTypes(data: Record<string, unknown>[], fields: FieldMeta[]) {
  return data.map((row) => {
    ...
    fields.forEach((f) => { out[f.name] = f.inferredType === 'number' ? (Number(row[f.name]) || 0) : row[f.name] })
    return out
  })
}
...
export default function DataVisualizerPage() {
  ...
  const parseExcelFile = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      } catch (err) { setError(`Excel 解析失败: ${(err as Error).message}`) }
    }
  ...
  const handleFileSelect = (file: File | undefined) => {
    ...
    if (name.endsWith('.csv')) parseCSVFile(file)
    else if (name.endsWith('.xlsx') || name.endsWith('.xls')) parseExcelFile(file)
    else setError('不支持的文件格式，请上传 .csv / .xlsx / .xls 文件')
  }

  const handleDrop = (e: React.DragEvent) => { e.preventDefault(); setDragOver(false); handleFileSelect(e.dataTransfer.files[0]) }
  ...
  const parseInput = () => {
    ...
    try {
      if (trimmed.startsWith('[') || trimmed.startsWith('{')) {
        processParsedData(Array.isArray(JSON.parse(trimmed)) ? JSON.parse(trimmed) : [JSON.parse(trimmed)])
        return
      }
      const result = Papa.parse<Record<string, string>>(trimmed, { header: true, skipEmptyLines: true })
      if (result.errors.length > 0) { setError(`CSV 解析错误: ${result.errors[0].message}`); return }
      processParsedData(result.data as Record<string, unknown>[])
    } catch (e) { setError(`解析失败: ${(e as Error).message}`) }
  }
  ...
  const toggleAllYFields = () => {
    const allNumberFields = fields.filter(f => f.inferredType === 'number').map(f => f.name)
    setYFields(prev =>
      prev.length === allNumberFields.length ? [] : allNumberFields
    )
  }
  ...
  const option = ((): echarts.EChartsOption | null => {
    ...
    if (chartType === 'radar') {
      const indicator = yFields.map((y) => ({
        name: y,
        max: Math.max(...parsedData.map((r) => Number(r[y] ?? 0))) * 1.2 || 1,
      }))
      const seriesData = parsedData.map((row, i) => ({
        name: String(row[xField] ?? `系列${i + 1}`),
      ...
      return {
        ...
        radar: { indicator, shape: 'circle' as const, center: ['50%', '52%'], radius: '62%' },
    ...
    if (chartType === 'heatmap') {
      ...
      const xVals = [...new Set(categories)]
      const groupVals = groupField ? [...new Set(parsedData.map((r) => String(r[groupField])))] : ['value']
  ...
  const clearAll = () => {
    ...
    // ✅ 图表会在 option 变 null 后自动 dispose（通过 renderChart 的 useEffect）
  }

  const numberFields = fields.filter(f => f.inferredType === 'number')
  const allNumberFieldNames = numberFields.map(f => f.name)
  const isAllYSelected = yFields.length === allNumberFieldNames.length && allNumberFieldNames.length > 0
    ...
    <div className="max-w-[1400px] mx-auto py-12 px-4 sm:px-6 lg:px-10">
      ...
      <div className="grid lg:grid-cols-[400px_1fr] gap-6">
        ...
        <div className="space-y-4">
          <div className="border border-app-border rounded-2xl p-4 bg-app-bg space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-1.5 flex-wrap">
                ...
                <button onClick={() => loadSample('radar')} className="px-2.5 py-1 text-xs bg-violet-50 text-violet-600 rounded-lg hover:bg-violet-100 transition">雷达</button>
              </div>
            </div>

            <div ref={dropRef} onDrop={handleDrop} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onClick={() => fileInputRef.current?.click()} className={`border-2 border-dashed rounded-xl p-4 text-center transition-all cursor-pointer ${dragOver ? 'border-violet-500 bg-violet-50' : 'border-gray-300 hover:border-violet-300 hover:bg-gray-50'}`}>
              <input ref={fileInputRef} type="file" accept=".csv,.xlsx,.xls" onChange={(e) => handleFileSelect(e.target.files?.[0])} className="hidden" />
            ...
            <textarea value={rawInput} onChange={(e) => setRawInput(e.target.value)} placeholder="粘贴 CSV 或 JSON 数组..." className="w-full h-28 px-3 py-2 border border-gray-300 rounded-xl font-mono text-xs focus:outline-none focus:ring-2 focus:ring-violet-400 resize-none" spellCheck={false} />
            ...
            {parsedData.length > 0 && <div className="text-xs text-emerald-600 font-medium">✅ 已解析 {parsedData.length} 行 × {fields.length} 列</div>}
          </div>
          ...
          {fields.length > 0 && (
            <div className="border border-app-border rounded-2xl p-4 bg-app-bg space-y-3">
              ...
              {/* ✅ Y 轴复选框（替代 select multiple） */}
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="text-xs text-app-muted">
                    Y 轴（数值字段）
                    {isRadar && <span className="text-violet-600 font-medium"> — 雷达图需选择 3 个以上字段才能形成多边形</span>}
                  </label>
                  <button onClick={toggleAllYFields} className="text-xs text-violet-600 hover:text-violet-800 font-medium transition-colors">
                    {isAllYSelected ? '取消全选' : '全选'}
                  </button>
                </div>
                <div className="border border-gray-300 rounded-xl p-2 max-h-40 overflow-y-auto space-y-1 bg-white">
                  {numberFields.map((f) => (
                    <label key={f.name} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-violet-50 cursor-pointer transition-colors">
                      <input
                        type="checkbox"
                        checked={yFields.includes(f.name)}
                        onChange={() => toggleYField(f.name)}
                        className="w-4 h-4 rounded border-gray-300 text-violet-600 focus:ring-violet-500 accent-violet-500"
                      />
                      <span className="text-sm text-gray-700">{f.name}</span>
                    </label>
                  ))}
                </div>
                {isRadar && yFields.length < 3 && (
                  <div className="text-xs text-amber-600 mt-1">⚠ 雷达图至少需要选择 3 个 Y 轴字段才能正常显示多边形</div>
                )}
              </div>
        ...
        <div className="space-y-4">
          <div className="border border-app-border rounded-2xl p-4 bg-app-bg">
            ...
            {/* ✅ 没数据时显示占位 */}
            {parsedData.length === 0 ? (
              <div className="w-full h-[480px] rounded-xl bg-gray-50 flex items-center justify-center text-gray-400 text-sm">
                暂无数据，请上传文件或粘贴数据后解析
              </div>
            ) : (
      ...
      <div className="mt-10 p-4 bg-gray-50 border border-app-border rounded-xl">
        ...
        <ul className="text-xs text-app-muted space-y-1.5 leading-relaxed">
          ...
          <li>• Y 轴点击复选框选择字段，雷达图需选 3 个以上字段才能形成多边形</li>
          <li>• 选择 X 轴（分类）和 Y 轴（数值），可选分组字段拆分系列</li>
